...
namespace Krak\DoctrineUtil\ORM\Mapping\Driver;
...
/**
 * Clone of the standard DatabaseDriver but allows for filtering of the schema before getting table info. This is
 * useful when you only want to export a part of a very large db
 */
final class SchemaFilteringDatabaseDriver implements MappingDriver
{
    /**
    ...
    /**
     * Optional filter to apply when listing tables to map
     *
     * @var array
     */
     ...
     */
    public function __construct(AbstractSchemaManager $schemaManager, array $regexFilters = [])
    {
    ...
    private function listTableNames(): iterable {
        $tableNames = $this->_sm->listTableNames();
        if (!$this->regexFilters) {
            yield from $tableNames;
            return;
        }

        foreach ($tableNames as $tableName) {
            foreach ($this->regexFilters as $regexFilter) {
                if (preg_match($regexFilter, $tableName) === 1) {
                    yield $tableName;
                }
            }
        }
    ...
    private function reverseEngineerMappingFromDatabase()
    {
        ...
        // SG: ony change to the entire class is right here
